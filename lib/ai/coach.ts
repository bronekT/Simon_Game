...
дай тёплый, конкретный, практичный разбор. Замечай ПРОГРЕСС (что улучшилось или
...
ЯЗЫК (важно): все ПОЯСНЕНИЯ, разбор, ситуации и названия — НА РУССКОМ. НО любые
слова, которые он РЕАЛЬНО ГОВОРИТ КЛИЕНТУ, — строго НА АНГЛИЙСКОМ, потому что
клиенты в Онтарио говорят по-английски. То есть: поле "say" — на АНГЛИЙСКОМ;
"product_tips" (что сказать про продукт) — на АНГЛИЙСКОМ; в "tricks.how" пояснение
на русском, но если приводишь пример реплики — саму реплику на английском (в
кавычках).

Верни ТОЛЬКО JSON (без markdown, без кода, без пояснений вокруг):
{
  "progress": "2–3 предложения на русском о динамике: что стало лучше/хуже",
  "strengths": ["до 4 сильных сторон (на русском), которые помогают закрывать"],
  "improve": ["до 4 повторяющихся ошибок / над чем работать (на русском)"],
  "phrases": [{"when":"ситуация коротко (РУССКИЙ)","say":"exact words to say to the client (ENGLISH)"}],
  "tricks": [{"name":"название техники (РУССКИЙ)","how":"как применять, 1 предложение (РУССКИЙ; пример реплики — на ENGLISH в кавычках)"}],
  "product_tips": ["a ready product line to SAY to the client (ENGLISH)"]
}
БУДЬ КРАТОК: каждый пункт — ОДНО короткое предложение. Не больше: strengths 4,
improve 4, phrases 4, tricks 3, product_tips 3. Без воды. Обязательно заверши JSON
корректно.`;
