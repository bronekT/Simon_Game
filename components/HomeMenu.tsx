...
export function HomeMenu() {
  ...
  return (
    <details className="relative z-50">
      ...
      <div className="glass-strong absolute right-0 z-50 mt-2 w-48 overflow-hidden rounded-card border border-hairline shadow-2xl">
        {items.map((it) => (
          <Link key={it.href} href={it.href} className="block px-4 py-3 text-sm font-medium text-text transition active:bg-accent/15">
            {it.label}
          </Link>
        ))}
        <form action="/auth/signout" method="post" className="border-t border-hairline">
          <button type="submit" className="block w-full px-4 py-3 text-left text-sm text-risk transition active:bg-risk/10">
            Sign out
          </button>
        </form>
      </div>
